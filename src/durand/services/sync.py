...
class Sync:
    ...
    def _update_cob_id(self, value):
        self._node.adapter.remove_subscription(cob_id=self._cob_id, callback=self._receive_sync)
        self._cob_id = value & 0x1FFF_FFFF
        self._node.adapter.add_subscription(cob_id=self._cob_id, callback=self._receive_sync)
