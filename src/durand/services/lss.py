...
class LSS:
    ...
    def cmd_switch_mode_selective(self, msg: bytes):
        ...
        self._received_selective_address[index] = int.from_bytes(msg[1:5], 'little', signed=False)
    ...
    def cmd_configure_bit_timing(self, msg: bytes):
        ...
        if selector != 0 or index not in valid_table_entries or self._change_baudrate_cb is None:
    ...
    def cmd_identify_remote_slaves(self, msg: bytes):
        ...
        if (vendor == self._remote_slave_address[0] and
                ...
                self._remote_slave_address[4] <= serial <= self._remote_slave_address[5]):
