...
class UploadManager:
    ...
    def init_upload(self, msg: bytes):
        ...
        if msg[0] & 0xE3 == 0xA0 and (size is None or size > msg[5]):  # ccs=5, cs=0 -> block upload
            ...
            self._server.node.adapter.send(self._server.cob_tx, cmd.to_bytes(1, 'little') + msg[1: 4] + size_bytes)
            return
        ...
        if size is not None and size <= 4:
            ...
            response = SDO_STRUCT.pack(cmd, index, subindex) + data + bytes(4 - len(data))
    ...
    def upload_segment(self, msg: bytes):
        ...
        self._server.node.adapter.send(self._server.cob_tx, cmd.to_bytes(1, 'little') + data + bytes(7 - len(data)))

    def upload_sub_block(self, msg: bytes):
        if self._state != TransferState.BLOCK:
            self._abort()
            raise SDODomainAbort(0x05040001, variable=None)  # client command specificer not valid
        ...
        if msg[0] & 0x03 == 2:
            ...
            if not self._stream.peek(1):
                cmd = 0xC1 + ((7 - len(data) % 7) << 1)
                crc = self._crc if self._crc is not None else 0
                self._server.node.adapter.send(self._server.cob_tx, cmd.to_bytes(1, 'little') + crc.to_bytes(2, 'little') + bytes(5))
                self._state = TransferState.BLOCK_END
                return
        
        data = self._stream.peek(self._block_size * 7 + 1)
        data, following_byte = data[:self._block_size * 7], data[self._block_size * 7:]
        ...
        first_byte = (last_block << 7) + len(chunks)
        response = first_byte.to_bytes(1, 'little') + chunks[-1] + bytes(7 - len(chunks[-1]))
        self._server.node.adapter.send(self._server.cob_tx, response)
        
    def upload_block_end(self, msg: bytes):
